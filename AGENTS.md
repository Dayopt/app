...
- **repo 全体を洗う検索では `rg --hidden` を付ける。** `rg` は既定で dot ディレクトリを飛ばすため、`.claude/` `.codex/` `.agents/` `.github/` が丸ごと検索対象から外れる。撤去・改名の残存参照を探す時にこれを忘れると、AI 設定と workflow の参照だけが取り残される（2026-08-03 に実際に発生。`.git/**` は除外する）
