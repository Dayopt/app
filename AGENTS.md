...
| #   | ルール                                                 | 種別       | 失効条件（この前提が変わったら再検証する）           |
| --- | ------------------------------------------------------ | ---------- | ---------------------------------------------------- |
| 1   | **個人の 1 日が良くならないなら、作らない**            | 境界       | Target が個人ユーザーから変わった時                  |
| 2   | **迷ったら、計画と実績の距離を縮める方を選ぶ**         | 優先順位   | Differentiator が「一体化」から変わった時             |
| 3   | **Google Calendar / Toggl より一手少なく**             | 方法       | 比較対象が変わった時、または両ツールが UI を大改訂した時 |
| 4   | **不可逆だけ遅く、可逆は速く**                         | タイミング | pre-launch を抜けて外部契約が増え、可逆の範囲が狭まった時 |
| 5   | **2 週間、自分が触らなかった機能は削除候補にする**     | 停止       | ユーザーが増え、作者の使用パターンが代表性を失った時   |
...
5. Todo エンジンを**一度作って捨てた**（strategy.md §5-2）。作る前に止める判断が効かなかった実例がある。既定を「改善リスト行き」から「削除リスト行き」へ反転させる

### 執行設計

- **ルール 3 / 4 / 5 は機械判定できる。Main がゲートとして厳守する**
  - 3: UI フローを含む plan に操作数比較を書く（`.claude/rules/plan-format.md`）
  - 4: Reversibility Table（同上）
  - 5: 月次ガーデニングの問い（`.claude/commands/gardening.md`）
- **ルール 1 / 2 は価値判断。Main は判定せず「問いの強制」を行う**
  - CHECKPOINT で必ず発話する。黙って通さない
  - ユーザーは override してよい。ただし理由を一文残す
  - 禁止するのはルール違反ではなく**無言の**ルール違反

### メタルール

**6 個目を足すときは、どれかを削る。** 少数であることがこのルール群の機能なので、増やすなら等価交換にする。
