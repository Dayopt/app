...
>(({ className, ...props }, ref) => {
  return (
    <SheetPrimitive.Overlay
      ...
      className={cn(
        'data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 bg-overlay z-sheet fixed inset-0 backdrop-blur-sm data-[state=closed]:pointer-events-none motion-reduce:animate-none',
        className,
      )}
      {...props}
    />
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          'bg-card text-card-foreground z-sheet fixed flex flex-col gap-4 shadow-lg transition ease-in-out motion-reduce:transition-none',
