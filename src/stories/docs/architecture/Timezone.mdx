...
<Meta title="Docs/Architecture/Timezone" />
...
| 環境                    | ソース                                        | フォールバック                                     |
| ----------------------- | --------------------------------------------- | -------------------------------------------------- |
| **クライアント**        | `useCalendarSettingsStore((s) => s.timezone)` | `Intl.DateTimeFormat().resolvedOptions().timeZone` |
...
| `src/stores/useCalendarSettingsStore.ts`             | Layer 1: クライアントTZ保持                       |
