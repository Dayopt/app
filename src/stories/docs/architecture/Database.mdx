...
| テーブル           | 役割                                 | 主要カラム                                                                               |
| ------------------ | ------------------------------------ | ---------------------------------------------------------------------------------------- |
| **plans**          | タスク/プラン（タイムボクシング）    | title, status(open/closed), start_time, end_time                                         |
...
| **plan_instances** | プランの個別インスタンス（レガシー） | plan_id, instance_date, exception_type, title, description, instance_start, instance_end |
...
│ reminder_*        │  │   self, max 1階層) │  │ is_read          │
└───────┬──────────┘  └───────┬──────────┘  └──────────────────┘
        │                     │
   ┌────┼────┐           ┌────┘
   │    │    │           │
