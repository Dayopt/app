...
<Mermaid chart={`graph TD
    ...
    SVC --> SB
    SB --> PG
    PG -.->|"INSERT/UPDATE/DELETE 通知"| RT
    RT -.->|"キャッシュ invalidation"| TQ
    TQ -.->|"再レンダリング"| RC
`} />
...
<Mermaid chart={`graph TD
    ...
    P --> RP["RealtimeProvider"]
    P --> TP["ThemeProvider"]
    P --> GS["GlobalSearchProvider (lazy)"]
    P --> SW["ServiceWorkerProvider (lazy)"]
    P --> GT["GlobalTagCreateModal (lazy)"]
`} />
