 ...
 */
export function useScrollEntryIntoView({
  scrollContainerRef,
  hourHeight,
}: UseScrollEntryIntoViewOptions) {
  const isMobile = useMediaQuery(MEDIA_QUERIES.mobile);
  const inspectorEntryId = useEntryInspectorStore((s) => s.entryId);
  const inspectorIsOpen = useEntryInspectorStore((s) => s.isOpen);
  ...
  // tagDraft.tag.id を deps に含めて「別タグに切り替わった」場合だけ再 scroll する
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const [h, m] = tagDraft.startTime.split(':').map(Number);
    ...
    return () => clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- startTime/endTime の変化では re-scroll しない（tag 切替時のみ）
  }, [isMobile, tagDraft?.tag.id, hourHeight, scrollContainerRef]);
}
