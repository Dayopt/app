...
export const ByConvention: Story = {
  render: () => {
    const groups: Array<{
      title: string;
      note?: string;
      items: Array<{ icon: typeof Home; name: string; usage?: string }>;
    }> = [
      {
        title: 'Pro / 課金',
    ...
    const renderCell = ({
      ...
      <div
        key={`${name}-${usage ?? ''}`}
        className="border-border-subtle flex flex-col items-center gap-2 rounded-lg border p-4 text-center"
      >
