  ...
  data={{
    sections: {
      ...
      googleCalendar: {
        ...
        whatWeStore:
          "What we store — for each imported event, only its identifier, title, description, start and end time, whether it is active or cancelled, and the identifier and name of the calendar it came from. For a calendar someone shared with you, that identifier may be the sharing account's email address. Attendees, guest email addresses, locations, conferencing links, and attachments are never used, stored, or logged.",
        calendarSelection:
