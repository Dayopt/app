...
`apps/web`（マーケティングサイト・docs・blog）の Storybook レイヤー。
...
Storybook の top-level は**所有境界（package / app）**で分ける（ADR-023）。
...
物理構造 `components/ui/<category>/` と `Shared/Components`（ADR-022）の taxonomy に揃える。
