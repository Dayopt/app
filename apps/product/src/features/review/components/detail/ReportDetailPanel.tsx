 ...
 * アクティビティ詳細パネル（仕様 §6）。
 *
 * shell（`desktop-layout.tsx`）が用意した 4 カラム目へ portal する。**パネル内で編集はしない**
 * （充実の後付けは編集面の仕事）。「（記録中）」も作らない — Dayopt の Record は常に
 * `end_at <= now` で、記録中という状態が存在しない。
 *
...
function DetailBody({
  ...
  return (
    <>
      ...
      <TrendBars granularity={granularity} trend={detail.trend} />
