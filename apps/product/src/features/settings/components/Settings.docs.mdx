...
| weekStartsOn            | 週開始日                   |
| showWeekNumbers         | 週番号表示                 |
| defaultDuration         | 新規エントリの既定時間     |
| snapInterval            | ドラッグ操作のスナップ間隔 |

参照は `@/lib/hooks/useUserPreferences`、更新は `@/lib/hooks/useUpdateUserSettings` を使用する。
