...
const LEGACY_CANCELLABLE_SUBSCRIPTION_STATUSES = new Set([
  'active',
  'past_due',
  'paused',
  'trialing',
...
/**
 * Pre-activation compatibility path.
 *
 * This preserves the current production behavior while old app instances are
 * still serving requests. Provider identity fencing is introduced only by the
 * durable path after the shared activation checkpoint.
 */
export async function deleteLegacyBillingAccountData(userId: string): Promise<void> {
  ...
  const { data: profile, error } = await db
    ...
    .eq('id', userId)
    .maybeSingle();
  if (error) throw deletionFailure('legacy_profile', error);

  const stripeCustomerId = profile?.stripe_customer_id ?? null;
  if (stripeCustomerId === null) return;

  const stripe = getStripe();
  if (stripe === null) throw deletionFailure('legacy_configuration');

  try {
    const subscriptions = await stripe.subscriptions.list({ customer: stripeCustomerId });
    for (const subscription of subscriptions.data) {
      if (!LEGACY_CANCELLABLE_SUBSCRIPTION_STATUSES.has(subscription.status)) continue;
      await stripe.subscriptions.cancel(subscription.id);
    }
    await stripe.customers.del(stripeCustomerId);
  } catch (providerError) {
    throw deletionFailure('legacy_provider', providerError);
  }
}
