 ...
 *
 * アーカイブ済みアクティビティ / カテゴリーの参照・復元・完全削除の入口。
 * 1 件も無ければ何も描画しない。取得失敗時はこれらの唯一の UI が消えてしまわないよう、
 * 0 件（空）とは区別して ErrorState + リトライを表示する。
 */
export function ArchivedActivityList({ onDelete }: ArchivedActivityListProps) {
  const t = useTranslations();
  const [expanded, setExpanded] = useState(false);
  const { data: archived, isError, refetch } = useArchivedTags();
          ...
          {archived.map((item) => (
            ...
            >
              <span className="ml-2 shrink-0">
                <TagIcon icon={item.icon} color={item.color} size="sm" />
              </span>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" side="right">
                  <DropdownMenuItem onClick={() => restoreMutation.mutate({ id: item.id })}>
