...
const baseTimeModelSchema = z.object({
...
export const createPlanSchema = baseTimeModelSchema.superRefine(timeRangeRefine);

export const updatePlanSchema = baseTimeModelSchema.partial().superRefine(timeRangeRefine);
...
const baseLogSchema = baseTimeModelSchema.extend({
  planId: z.string().uuid().nullable().optional(),
  fulfillmentScore: z.number().int().min(1).max(3).nullable().optional(),
});

export const createLogSchema = baseLogSchema.superRefine(timeRangeRefine);

export const updateLogSchema = baseLogSchema.partial().superRefine(timeRangeRefine);
