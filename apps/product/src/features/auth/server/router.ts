...
export function createUserRouter(dependencies: UserServiceDependencies) {
  return createTRPCRouter({
    listOAuthConnections: protectedProcedure
      ...
      .query(async ({ ctx }) => {
        try {
          return await createUserService(ctx.supabase, dependencies).listOAuthConnections(
            ctx.userId!,
          );
        } catch (error) {
    ...
    revokeOAuthConnection: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          return await createUserService(ctx.supabase, dependencies).revokeOAuthConnection(
            ctx.userId!,
            input.connectionId,
          );
        } catch (error) {
     ...
     * 全データを削除（アカウントは保持）
     */
    prepareDeleteAllData: protectedProcedure
      .meta({ description: '全データ削除のoperationを準備' })
      .mutation(async ({ ctx }) => {
        try {
          const service = createUserService(ctx.supabase, dependencies);
          return await service.prepareDeleteAllData(ctx.userId!);
        } catch (error) {
          return handleServiceError(error);
        }
      }),
    ...
    deleteAllData: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        try {
          const service = createUserService(ctx.supabase, dependencies);
          return await service.deleteAllData(
            ctx.userId!,
            input.operationId,
