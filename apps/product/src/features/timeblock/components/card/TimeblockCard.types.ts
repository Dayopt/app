...
/** TimeblockCardコンポーネントのPropsをまとめた型 */
export interface TimeblockCardProps {
  ...
  /** 外部（WeekContent等）がwrapper側でoverlay位置調整済みの場合true。TimeblockCard内でのtopShift/heightDelta適用をスキップする */
...
/** カレンダーグリッド上のTimeblockCardの位置情報（top/left/width/heightはpx or %） */
export interface TimeblockCardPosition {
  top: number; // px
  left: number; // %
  width: number; // %
  height: number; // px
