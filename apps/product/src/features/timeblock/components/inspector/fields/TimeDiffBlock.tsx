...
interface TimeDiffBlockProps {
  ...
  /** タグの色名（TimeblockCard と同じ色でバーを描画） */
...
export function TimeDiffBlock({
  ...
  if (plannedDuration <= 0) {
    ...
    return (
      <div
        ...
        role="img"
        aria-label={`${t('timeblock.inspector.time.actual')} ${formatDurationDisplay(actualDuration)}`}
      >
        <div className="flex items-center gap-1">
          <span className="text-muted-foreground text-sm tabular-nums">
            {t('timeblock.inspector.time.planned')} 0min / {t('timeblock.inspector.time.actual')}{' '}
  ...
  // aria-label
  const ariaLabel = `${t('timeblock.inspector.time.planned')} ${formatDurationDisplay(plannedDuration)} / ${t('timeblock.inspector.time.actual')} ${formatDurationDisplay(actualDuration)}`;
  ...
  return (
    <div className="flex flex-col gap-1" role="img" aria-label={ariaLabel}>
      ...
      <div className="flex items-center gap-1">
        <span className="text-muted-foreground text-sm tabular-nums">
          {t('timeblock.inspector.time.planned')} {formatDurationDisplay(plannedDuration)} /{' '}
          {t('timeblock.inspector.time.actual')} {formatDurationDisplay(actualDuration)}
        </span>
