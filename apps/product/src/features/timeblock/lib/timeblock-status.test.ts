 ...
 * - getTimeblockState: 時間位置から状態を判定 (upcoming/active/past)
 * - isTimeblockPast: エントリが過去かどうかを判定
 */

import { describe, expect, it } from 'vitest';

import { getTimeblockState, isTimeblockPast } from './timeblock-status';

describe('Entry Status Utilities', () => {
  // Fixed reference time for deterministic tests
  const referenceTime = new Date('2026-03-12T12:00:00Z');

  describe('getTimeblockState', () => {
    describe('null start/end times', () => {
      it('should return upcoming when start_time is null', () => {
        ...
        const result = getTimeblockState(entry, referenceTime);
    ...
    describe('active entry (start <= now < end)', () => {
      ...
      it('planned終了後でもactual超過中ならactive', () => {
        ...
        expect(getTimeblockState(entry, referenceTime)).toBe('active');
      });
    ...
    describe('default now parameter', () => {
      it('should use current Date when now is not provided', () => {
        ...
        const result = getTimeblockState(entry);
        expect(result).toBe('upcoming');
      });
    });
  });
  ...
  describe('isTimeblockPast', () => {
    it('should return true when entry is in past', () => {
      ...
      expect(isTimeblockPast(entry, referenceTime)).toBe(true);
    });
    ...
    it('should return false when entry is upcoming', () => {
      ...
      expect(isTimeblockPast(entry, referenceTime)).toBe(false);
    });
