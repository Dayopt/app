...
describe.skipIf(!RUN_LOCAL)('MCP Plan create apply integration', () => {
  ...
  it('revalidates global, Pro, token, and connection authority inside apply', async () => {
    const authorization = await createWriteAuthorization();
    ...
    const disabledConnectionAuthorization = await createWriteAuthorization();
    await admin.rpc('disable_oauth_connection_writes_v1', {
      p_connection_id: disabledConnectionAuthorization.connectionId,
    });
    const disabledConnection = await applyPlan(disabledConnectionAuthorization, {
      ...baseInput,
      operationId: crypto.randomUUID(),
    });
    expect(disabledConnection.error?.code).toBe('DM004');
