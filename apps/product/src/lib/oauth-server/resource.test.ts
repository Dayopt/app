...
describe('OAuth MCP resource normalization', () => {
  ...
  it('OAuth有効Previewは束縛されたbranch resourceだけを受理する', () => {
    const previewHost = 'product-git-codex-mcp-preview-dayopt.vercel.app';
    vi.stubEnv('MCP_OAUTH_ENVIRONMENT', 'preview');
    vi.stubEnv('MCP_OAUTH_PREVIEW_BRANCH', 'codex/mcp-preview');
    vi.stubEnv('OAUTH_AUTHORIZATION_SERVER_URI', `https://${previewHost}`);
    vi.stubEnv('MCP_CANONICAL_RESOURCE_URI', `https://${previewHost}`);
    vi.stubEnv('VERCEL_ENV', 'preview');
    vi.stubEnv('VERCEL_TARGET_ENV', 'preview');
    vi.stubEnv('VERCEL_BRANCH_URL', previewHost);
    vi.stubEnv('VERCEL_GIT_COMMIT_REF', 'codex/mcp-preview');

    expect(resolveRequestedResource(`https://${previewHost.toUpperCase()}:443/`)).toBe(
      `https://${previewHost}`,
    );
