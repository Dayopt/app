...
`pnpm dev` は `.op-env.local` の存在を確認し、`.env.local` / `apps/product/.env.local` / `apps/web/.env.local` が残っている場合は fail する。素の起動が必要な一時作業だけ `pnpm dev:raw` を使う。値の存在確認は値そのものを表示しない方法だけを使う。
