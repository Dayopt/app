...
Secrets の正本は `apps/storybook/docs/operations/secrets.mdx`。1Password は production / shared / optional staging の長寿命 secrets だけを管理する。PR Preview Branch credentials は 1Password に保存せず、Supabase / Vercel integration の ephemeral replica として扱う。
...
詳細は `apps/storybook/docs/dev/guides/MigrationChecklist.mdx`。
