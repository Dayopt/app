...
├── site-key     (text, public)
└── secret-key   (concealed, web verify / Supabase Auth 用)
```
...
- **app** — `.op-env.local`:
  ```
  NEXT_PUBLIC_TURNSTILE_SITE_KEY=op://Dayopt-Shared/turnstile/site-key
  ```
  （app は secret を持たない）
- **web** — `.op-env.local`:
  ```
  NEXT_PUBLIC_TURNSTILE_SITE_KEY=op://Dayopt-Shared/turnstile/site-key
  TURNSTILE_SECRET_KEY=op://Dayopt-Shared/turnstile/secret-key
  ```
- **Supabase Auth** — Dashboard → Authentication → Bot & Abuse Protection → Turnstile
  - `Secret key` に `Dayopt-Shared/turnstile/secret-key` の値を貼る
...
2. 1Password `Dayopt-Shared/turnstile/{site-key,secret-key}` を更新
3. Supabase Auth dashboard の secret key を差し替え
4. アプリ再デプロイは**不要**（op 経由で次回起動時に新値が注入される）

### 開発時フォールバック

Cloudflare 公式の dev 用テストキー（常時 pass）:
...
`.op-env.local` の該当行を**一時的に**ハードコードキーに差し替えるとオフラインでも widget テストができる。commit 前に必ず元の `op://` 参照に戻すこと。

その他のテストキー:

| 用途           | site-key                   | secret-key                            |
| -------------- | -------------------------- | ------------------------------------- |
| 常時 pass      | `1x00000000000000000000AA` | `1x0000000000000000000000000000000AA` |
| 常時 block     | `2x00000000000000000000AB` | `2x0000000000000000000000000000000AA` |
| 強制 challenge | `3x00000000000000000000FF` | `1x0000000000000000000000000000000AA` |
