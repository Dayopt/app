...
<Meta title="Architecture/API First Audit" />
...
target shape の decide は [api-shape.md](?path=/docs/architecture-api-shape--docs) に分離。current vs target の delta もそこに集約。
