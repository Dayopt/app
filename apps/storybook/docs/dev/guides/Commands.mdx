...
> **Secrets**: 実値は `.env.local` に置かず、1Password master と `.op-env.local` の `op://` 参照を `pnpm dev` で注入する。素の起動が必要な一時作業だけ `pnpm dev:raw` を使う。詳細は `apps/storybook/docs/operations/secrets.mdx`。
...
```bash
npm run env:check           # secret 値を表示せず env の存在確認
npm run vercel:env          # Vercel 環境変数一覧
npm run vercel:env:pull:unsafe  # apps/product/.env.local に一時同期
```
