-- Keep the no-strategy safety check and child promotion inside the same
-- transaction as tag/timeblock deletion. The application may still preflight
-- for UX, but this command is the final authority under the user lock.

CREATE FUNCTION public.delete_tags_with_timeblocks_command_v2(
  p_user_id UUID,
  p_tag_ids UUID[],
  p_strategy TEXT,
...
GRANT EXECUTE ON FUNCTION public.delete_tags_with_timeblocks_command_v2(
  UUID, UUID[], TEXT, UUID, BOOLEAN
) TO service_role;

-- The v1 public wrapper cannot express require-empty or atomic child promotion.
REVOKE ALL ON FUNCTION public.delete_tags_with_timeblocks_command_v1(
  UUID, UUID[], TEXT, UUID
) FROM PUBLIC, anon, authenticated, service_role;
DROP FUNCTION public.delete_tags_with_timeblocks_command_v1(UUID, UUID[], TEXT, UUID);

COMMENT ON FUNCTION public.delete_tags_with_timeblocks_command_v2(
  UUID, UUID[], TEXT, UUID, BOOLEAN
) IS 'Atomically revalidates deletion intent, promotes surviving children when requested, applies the association strategy, and deletes tags under an exclusive same-user lock; service role only.';
