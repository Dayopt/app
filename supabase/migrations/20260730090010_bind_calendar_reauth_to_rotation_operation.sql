-- Resolve an ambiguous committed token rotation without letting a stale
-- fallback overwrite a reconnect. Operation identity is accepted only when
-- the row also owns the exact new ciphertext from that operation.
...
DROP FUNCTION public.mark_calendar_connection_reauth_command_v1(
  UUID, UUID, BIGINT, TEXT, TIMESTAMPTZ
);

CREATE FUNCTION public.mark_calendar_connection_reauth_command_v2(
  p_user_id UUID,
...
BEGIN
  ...
  IF v_current_refresh_token_enc IS DISTINCT FROM p_expected_refresh_token_enc
    AND NOT (
      p_operation_id IS NOT NULL
      AND v_current_operation_id IS NOT DISTINCT FROM p_operation_id
      AND v_current_refresh_token_enc IS NOT DISTINCT FROM p_new_refresh_token_enc
    ) THEN
    RETURN 'superseded';
  END IF;
...
REVOKE ALL ON FUNCTION public.mark_calendar_connection_reauth_command_v2(
  UUID, UUID, BIGINT, TEXT, UUID, TEXT, TIMESTAMPTZ
) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.mark_calendar_connection_reauth_command_v2(
  UUID, UUID, BIGINT, TEXT, UUID, TEXT, TIMESTAMPTZ
) TO service_role;

COMMENT ON FUNCTION public.mark_calendar_connection_reauth_command_v2(
  UUID, UUID, BIGINT, TEXT, UUID, TEXT, TIMESTAMPTZ
) IS
  'Marks only the observed or exact operation-owned Calendar token authority reauth_required and reports marked, missing, or superseded; service role only.';
