-- #2055(a): 20260813130100 added per-candidate subtransaction isolation to
-- finalize_calendar_revoke_guards_internal_v1 so one bad row is deferred
-- instead of aborting the whole batch, matching the pattern expire already
-- had. That fix's side effect is the same the risk-reviewer flagged when it
-- was made: deferred/isolated rows have no observation surface — the
-- RAISE WARNING only reaches logs, and cron.job_run_details only records
-- success/failure of the whole run. A row stuck in revoke_guarded /
-- expiry_guarded past its guard window could retry silently forever.
--
-- This adds a backlog counter to the existing maintenance status RPC
-- (public.get_external_authority_maintenance_status_v1, last redefined by
-- 20260730090005) so external-connection-maintenance's dispatcher can see
-- it. `CREATE OR REPLACE FUNCTION` cannot add a column to an existing
-- RETURNS TABLE — PostgreSQL treats that as a return-type change (42P13) —
-- so this must DROP then CREATE, reissuing REVOKE/GRANT/COMMENT.
--
-- 24 hours is a threshold, not a guarantee: finalize runs hourly (:15), so a
-- row still guarded 24h later has missed roughly 24 finalize ticks and is
-- almost certainly stuck rather than merely awaiting its next tick.

BEGIN;

DROP FUNCTION public.get_external_authority_maintenance_status_v1();
...
RETURNS TABLE (
  ...
  calendar_finalize_stuck_count BIGINT
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
  ...
  SELECT
    ...
    (
      ...
      WHERE operation.state IN ('revoke_guarded', 'expiry_guarded')
        AND operation.guard_until < v_now - INTERVAL '24 hours'
    );
END;
$$;
...
COMMENT ON FUNCTION public.get_external_authority_maintenance_status_v1() IS
  'Returns aggregate-only unleased/expired revoke backlog, retention due flags, and stuck finalize-guard count without IDs or payloads; service role only.';
