...
-- retention for MCP/OAuth authority. The application still uses purge v2; this
-- migration only expands the DB-first maintenance surface.
...
CREATE INDEX oauth_authorization_codes_retention_idx
  ON public.oauth_authorization_codes (
    (COALESCE(consumed_at, expires_at)),
    code_hash
  );

CREATE INDEX oauth_tokens_access_retention_idx
...
CREATE INDEX oauth_tokens_refresh_retention_idx
  ON public.oauth_tokens (
    (
      LEAST(
        expires_at,
        COALESCE(rotated_at, 'infinity'::TIMESTAMPTZ),
...
CREATE INDEX oauth_connections_retention_idx
  ON public.oauth_connections (
    (
      CASE
        WHEN revoked_at IS NULL THEN reauth_required_at
        WHEN revoked_at < reauth_required_at THEN revoked_at
...
-- =============================================================================
-- 3. OAuth and security-event retention cleanup
-- =============================================================================
...
BEGIN
  ...
  IF p_limit IS NULL OR p_limit < 1 OR p_limit > 10000 THEN
    RAISE EXCEPTION 'Authorization code cleanup limit must be between 1 and 10000'
      USING ERRCODE = '22023';
  END IF;
...
COMMENT ON FUNCTION public.cleanup_oauth_authorization_codes_v1(INTEGER) IS
  'Deletes consumed or expired OAuth authorization codes after 24 hours in a bounded batch; service role only.';
...
BEGIN
  ...
  IF p_limit IS NULL OR p_limit < 1 OR p_limit > 10000 THEN
    RAISE EXCEPTION 'Access token cleanup limit must be between 1 and 10000'
      USING ERRCODE = '22023';
  END IF;
...
COMMENT ON FUNCTION public.cleanup_oauth_access_tokens_v1(INTEGER) IS
  'Deletes expired or revoked OAuth access-token hashes after 24 hours in a bounded batch; service role only.';
...
BEGIN
  ...
  IF p_limit IS NULL OR p_limit < 1 OR p_limit > 10000 THEN
    RAISE EXCEPTION 'Refresh token cleanup limit must be between 1 and 10000'
      USING ERRCODE = '22023';
  END IF;
...
COMMENT ON FUNCTION public.cleanup_oauth_refresh_tokens_v1(INTEGER) IS
  'Deletes expired, rotated, or revoked OAuth refresh-token hashes after 30 days in a bounded batch; service role only.';
...
COMMENT ON FUNCTION public.cleanup_oauth_connections_v1(INTEGER) IS
  'Deletes revoked or reauthorization-expired OAuth connections after 90 days in a bounded batch; service role only.';
