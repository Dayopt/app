-- Align purge v3 with the multi-token revoke outbox. A purge locks each current
-- Calendar connection once and deletes it in the same transaction, so no
-- conflict suppression is needed or safe here.
...
COMMENT ON FUNCTION public.delete_all_user_data_command_v3(UUID) IS
  'Expanded atomic account-preserving purge with generation advance, locked multi-token Calendar revoke capture, MCP revocation, reports, and mirrors; service role only.';
