...
- **symbol 主張**: 「特定の関数 / hook / component を流用する」
...
3. **table / column 主張** → schema source を `Glob` / `Grep` で発見して検索。live DB 照合が必要なら Main に command / MCP 確認を依頼
4. **API 主張** → `Grep` で procedure / RPC 名を確認
5. **件数主張** → `Glob` / `Grep` の結果を実カウントと比較。toolで数えられなければ Main に read-only command を返す
...
- **変更しない**: file edit、issue comment、DB / external mutation、nested agent を実行しない

# 速度方針

- 1 主張あたり 1-2 tool call で済ませる。深追いしない
- 並列に Read / Grep を投げて待ち時間を圧縮する
- live source が必要でも external tool が使えない場合は、current repo source を確認し、live 照合を `⚠` として Main へ返す
