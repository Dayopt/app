 ...
 *   （path フィルタのみの言及を誤って ci 判定してしまうケースが実在する。
 *   例: `scripts/lib/storage-objects-app-policy-names.mjs` は
 *   `integration.yml` の paths: にのみ現れ、実行はされない。個々の判定結果は
 *   人間 / AI が最終確認すること — このライブラリは一次スクリーニングであり、
 *   確定判定ではない）
