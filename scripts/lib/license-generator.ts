/**
 * License Information Generator (shared core)
 *
 * apps/product・apps/web共通のOSSライセンス情報生成ロジック。
 * `pnpm licenses list --json` でproduction dependency treeのtransitive依存を
 * 含めて収集し、public向けのoss-credits.json / THIRD_PARTY_NOTICES.txtを生成する。
 *
...
export const GENERATED_FILE_NAMES = ['oss-credits.json', 'THIRD_PARTY_NOTICES.txt'] as const;

export type GeneratedFileName = (typeof GENERATED_FILE_NAMES)[number];
...
export interface RunLicenseGeneratorConfig {
  /** pnpm workspace上のpackage名 (例: '@dayopt/product') */
  packageName: string;
  /** workspaceRootからの相対path segments。cwdから特定できない場合のfallback (例: ['apps', 'product']) */
  fallbackPackageDirSegments: string[];
  /** packageDirから生成物の出力先を決める (例: (dir) => join(dir, 'public', 'legal')) */
  resolveOutputDir: (packageDir: string) => string;
  /** THIRD_PARTY_NOTICES.txtのフッターに載せる自スクリプトの相対path表記 */
  generatedByLabel: string;
}

/**
 * CLIエントリーポイントから呼ぶメイン処理
 */
export function runLicenseGeneratorCli(
  config: RunLicenseGeneratorConfig,
  ...
  const packageDir = resolvePackageDir(workspaceRoot, process.cwd(), config);

  // 1. pnpmでproduction dependency treeのライセンス情報を収集
  console.log('\n📦 Collecting dependency licenses...');
  const packages = collectLicenses(config.packageName, workspaceRoot);
  console.log(`   ✅ Found ${Object.keys(packages).length} packages`);

  // 2. 生成物を組み立てる
  const artifacts = buildLicenseArtifacts(
    packages,
    config.resolveOutputDir(packageDir),
    config.generatedByLabel,
  );
 ...
 */
function collectLicenses(packageName: string, workspaceRoot: string): Record<string, LicenseInfo> {
  const output = execFileSync(
    'pnpm',
    [
      '--filter',
      getProductionDependencyFilter(packageName),
      'licenses',
      'list',
      '--prod',
      '--json',
 ...
 */
export function generateNoticesFile(notices: string[], generatedByLabel: string): string {
...
function buildLicenseArtifacts(
  ...
  return {
    files: {
      ...
      'THIRD_PARTY_NOTICES.txt': generateNoticesFile(notices, generatedByLabel),
    },
    outputDir,
...
function resolvePackageDir(
  ...
  cwd: string,
  config: Pick<RunLicenseGeneratorConfig, 'packageName' | 'fallbackPackageDirSegments'>,
): string {
  const cwdPackageName = readPackageNameIfExists(cwd);
  if (cwdPackageName === config.packageName) {
    return resolve(cwd);
  }

  const packageDir = join(workspaceRoot, ...config.fallbackPackageDirSegments);
  if (readPackageNameIfExists(packageDir) === config.packageName) {
    return packageDir;
  }

  throw new Error(`Could not find ${config.packageName} package directory`);
}
