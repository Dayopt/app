...
3. **指揮台（Fable、着席時）** — 蒸留を読んで判断し、User への手作業依頼を出す
...
「Fable 向け蒸留」として、ready キュー / 走行中レーン / CI 状態 / 要判断事項を、**PR・issue 番号を保ったまま**整理する。判断・推奨・優先度付けは書かない（整理と事実の並べ替えまで）。「推奨」「優先」「すべき」等の語を含めない。
