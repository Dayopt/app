...
3. **判断層の検証**（`CLAUDE.md` §シンプルルール） — ①今月このルールに戻った場面はあったか（1 度も戻らないルールは削る候補）②無言で破られたルールは無いか（あれば今から理由を言語化）③先月触らなかった機能はどれか（ルール 5。削除候補は dispatch intake で起票）④**判断ジャーナル集計** — `pnpm gardening:judgment-journal`（`scripts/gardening/collect-judgment-journal.sh`）で現在ラベルが付く全分岐（issue / PR）を一覧化する。この script がやるのは列挙までで、結果を観測できた事例に判定コメントを追記してラベルを外す判断は人間 + 指揮台が行う。判定済み事例だけを母集団に `.claude/rules/orchestration.md` §権限の既定 の境界を実測で更新する（可逆な采配を Fable 決定 + opt-out にする試行運用の恒久化 / 巻き戻しの判定もここで行う）
