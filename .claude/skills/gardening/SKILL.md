...
3. **判断層の検証**（`CLAUDE.md` §シンプルルール） — ①今月このルールに戻った場面はあったか（1 度も戻らないルールは削る候補）②無言で破られたルールは無いか（あれば今から理由を言語化）③先月触らなかった機能はどれか（ルール 5。削除候補は dispatch intake で起票）④**決定ログの同期** — `pnpm decisions:sync`（`scripts/gardening/sync-decisions.mjs`、2026-08-20 に旧 `state:generate` から分離。[#2259](https://github.com/Dayopt/dayopt/issues/2259)。2026-08-27、[#2423](https://github.com/Dayopt/dayopt/issues/2423) の個別判定前倒しに伴い検索対象を `judgment:diverged` **と** `judgment:judged` の両ラベルへ拡張。`.claude/rules/orchestration.md` §判断ジャーナル 参照）を実行し、両ラベルが付く全分岐を `docs/decisions.md`（append-only 全履歴）へ追記する。sync 完了を確認したら、**`judgment:judged` が付く issue / PR からそのラベルを外す**（`judgment:diverged` は sweep 対象として残す。下記⑤）。順序を誤る（先にラベルを外す）と、その回で解決した分岐が append-only ログから永久に欠落する。⑤**判断ジャーナルの sweep + 境界集計** — `pnpm gardening:judgment-journal`（`scripts/gardening/collect-judgment-journal.sh`、同じく両ラベル対象）で**現在もラベルが付いたままの**全分岐（issue / PR）を一覧化する。`judgment:judged` は④で処理済みのため、ここに残るのは `judgment:diverged` のまま——日次運用が判定し損ねた書き漏れ、または判定材料がまだ揃っていない未観測の事例のみ（想定件数はゼロに近いはず）。観測できた事例には判定コメントを追記して `judgment:judged` へ付け替える（外すのは次回以降の④の sync 後）。判定済み事例（今回新たに判定したものを含む全件）だけを母集団に `.claude/rules/orchestration.md` §権限の既定 の境界を実測で更新する（可逆な采配を指揮台決定 + opt-out にする試行運用の恒久化 / 巻き戻しの判定もここで行う）
