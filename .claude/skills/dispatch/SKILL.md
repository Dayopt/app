...
- [ ] **ランダム抽出監査**（2026-08-20、[#2273](https://github.com/Dayopt/dayopt/issues/2273)。候補提示は 2026-08-21 に夜勤へ移管、[#2291](https://github.com/Dayopt/dayopt/issues/2291)）: 候補 PR の選定・提示は夜勤 Routine（`.claude/skills/night-watch/SKILL.md` §自動パート Step 4）が当日盤面 issue へ「DoD監査候補: #NNNN」として既にコメント済み。朝の編成では指揮台がこのコメントを確認し、対象 issue の dispatch コメントに記載した DoD（操作 A 手順 6）と実際の merge 内容が意図どおりかを User が 5 分で監査する。夜勤が「前日merge PR無し」を報告した日、または夜勤自体が故障している日は指揮台が候補提示を代行する。見つかったズレは `judgment:diverged` ラベルでジャーナル化する（`.claude/rules/orchestration.md` §判断ジャーナル）
