...
5. issue 本文を **handoff-quality** に補強する（下記テンプレート）。**束が CLAUDE.md §Codex（別系統批評係）の利用 A（設計レビュー）の発動条件に該当する場合は、ここで Codex 設計レビューを実行し採用分を本文へ反映してから次へ進む**（判定結果は手順 6 の dispatch コメントに「Codex A: 該当/非該当」で残す。選別基準・実行コマンドは CLAUDE.md §Codex を参照し複製しない）。worker が repo 探索なしで着手できる密度が基準
...
2. 重複なら既存 issue に本文追記 or コメントで統合。新規なら handoff-quality で起票（**RLS ポリシー・テナント境界・スキーマ変更に関わる起票では、ここで CLAUDE.md §Codex（別系統批評係）の利用 B（攻撃シナリオ生成）を実行し、出力を「## テストすべき攻撃シナリオ」として本文に貼る**。実行コマンドは CLAUDE.md §Codex を参照し複製しない）
...
- [ ] **heavy-post-merge の赤確認**（2026-08-20、CI 4 層再設計 [#2269](https://github.com/Dayopt/dayopt/issues/2269)。2026-08-25、[#2382](https://github.com/Dayopt/dayopt/issues/2382) で per-merge 実行を廃止）: `gh run list --workflow=heavy-post-merge.yml --limit 5 --json conclusion,createdAt,event` で直近 run を確認する。red（`conclusion=failure`）があれば修正 issue を最優先で起票する。E2E / Web E2E / Integration Tests は per-PR から撤去され、`heavy-post-merge.yml` は nightly + 手動発火でしか（`integration.yml` はこれに加え DB 系ファイルを触った main push でも）検証されないため、この確認を欠くと壊れた main が promote gate（層 4）に阻まれるまで無通知で滞留する。**直近 run が pending、または 24h より古い場合は完了を待って再確認する**（per-merge 廃止で「日中の main push run が backstop になる」前提が消えたため、nightly の遅延をそのまま見ると赤確認が空振りする）
