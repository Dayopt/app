...
- **draft 中に走るもの**: Docs Guard だけ（gitleaks による secret 検査は境界に関係なく早い方がよいため draft guard を持たない）。**Static Checks / Unit Tests は 2026-08-26 に draft から撤去した**（[#2415](https://github.com/Dayopt/dayopt/issues/2415)、User 裁可。指揮台の推奨とは分かれた `judgment:diverged`）。draft 中の機械ゲートは pre-push フック（§Pause point）
- **ready 後に走る層**: Static Checks / Unit Tests / Docs Guard + Production Config Audit（保護対象該当時のみ trusted dispatch）。**E2E / Web E2E は per-PR から撤去済み**（2026-08-20、CI 4 層再設計 #2269。§CI 4 層構造（2026-08-20 改訂） 参照）
...
- **skip は検査の免除ではない。** draft 中の Static / Unit は `conclusion: skipped` の check run を発行し、`branch:finish` が「docs-only でない PR では両者が success であること」を名前で要求する（`scripts/git/finish-branch.sh`、#2415）。skipped のまま merge へ抜ける経路は塞いである
...
| 層  | タイミング | 内容                                                                             | 実装                                                                           |
| --- | ---------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| 1   | draft push | Docs Guard のみ（Static Checks / Unit Tests は 2026-08-26 に撤去、#2415）        | `.github/workflows/docs-guard.yml`                                             |
| 2   | ready      | Static Checks / Unit Tests（Impact gate による docs-only skip 込み）+ Docs Guard | `.github/workflows/ci.yml`                                                     |
