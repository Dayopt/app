...
push 前の pause は git レベルの hook なので Claude / 人間 / wrapper script のすべてに効く。ただし DO-CONFIRM 部分は**スピードバンプであってゲートではない** — 機械が強制できるのは「観点を提示して 1 回止める」までで、答えたかどうかは検証できない。黙って再実行すれば通ってしまうことを前提に、答えを発話してから再実行するのは規律の側で守る。DO-CONFIRM の後に走る実行ゲート（typecheck/lint + typecheck:scripts/test:scripts/format:check）は本物のゲートで、失敗すれば push は通らない——ただし affected スコープの高速フィルタであり、CI Static Checks と同じ網ではない。

**このフックと CI の関係は 2026-08-26（[#2415](https://github.com/Dayopt/dayopt/issues/2415)、User 裁可）に改訂した。** 旧文言は「フックは CI に着く前に無料で落とす係で、フックが通ったことは CI の省略理由にならない」だったが、Draft 中の軽量 CI 廃止に伴い**段階で読み分ける**: **draft の間はフックが唯一の機械ゲート**（軽量 CI は skip される。§CI 4 層構造）で、**ready 以降は CI が正本**（フックが通ったことは ready 後の CI の省略理由にならない — ここは不変）。フックが構造的に捕まえられない差分（lockfile 不整合・未宣言依存など環境差に起因するもの）は ready 後の CI が受け持つ。この段階分けが割に合わなくなった場合の復活条件は `.github/workflows/ci.yml` 冒頭コメントに書いてある。turbo の `test:run` タスクは対象外にした（`apps/product`/`apps/web` の localStorage 依存テストが Node 26 環境で環境依存の失敗を起こす既知の pre-existing 事象があり、含めると push が恒常的に失敗しうるため）。
...
**`.husky/**` と `.claude/hooks/**` の変更はガードレールそのものであり、危険地帯扱いとする**（PR レビュー時に通常より厚く見る。plan 先行での変更が既定）。機械化できる CLAUDE.md 規律を見つけたら、月次見直しは `.claude/skills/gardening/SKILL.md` §人間パート 4⑦「機械へ昇格できる項目は無いか」で行う（新しい儀式は作らず既存項目へ接続する）。

規律は 3 つ。**①機械で強制できるものは機械へ**（止まるのが最強）。**②機械化できないものだけ発動点で明示発話する**。「該当なし」も言い切り、黙って通さない。**③項目を 1 つ足すときは 1 つ削る**。長いチェックリストは形骸化するのが最悪の失敗モードなので、各点 5〜9 項目に収める。この③は DO-CONFIRM の確認項目リストに対する規律で、push 前の実行ゲート（機械検証そのもの）の追加はこの制約の対象外——機械化は①の推奨する方向そのものであり、確認項目を増やすことと同一視しない。
...
### 束ねるのは編成の段階（2026-08-11 追記）

束ねは「PR を作る直前に思い出す規約」ではなく、**編成（チップ / レーンへの切り出し）の段階で決める**。指揮台は `status:ready` の issue をドメイン単位でまとめてから 1 レーンへ渡し、**そのバッチ = 1 branch = 1 PR** とする（`dispatch` skill 操作 A の束ね手順が入口）。レーンを起こしてから「関連 issue も入れるべきだった」と気づくと、branch を切り直すか PR を増やすかの二択になる。

**open PR は原則 1 本に保ち、直列で回す。** 並行 PR N 本は §なぜ束ねるか の up-to-date gate により追加 CI が O(N²) で効くため、本数を増やす判断は「独立して検証・revert したい」（下記の分割理由）に該当する時だけにする。

### docs-only の小粒 issue は指揮台が束ねる（2026-08-12 追記）

**docs-only の小粒 issue は単独 PR を立てず、指揮台が「docs 束」として溜めてから 1 レーン・1 branch・1 PR で流す。** 「サイズを理由に分割しない」の docs 版で、レーンが調査中に別の docs 修正を掘り当てるたびに issue 化 → 即 PR 化すると、1 issue = 1 PR に逆戻りして PR 固定費（§なぜ束ねるか）が積み増しになる。

2026-08-12、docs 系 PR が同日 2 本（#1976 / #1983）並び、さらに小粒 docs issue が 4 件（#1939 / #1964 / #1975 / #1980）滞留した事例が根拠。in-flight で既に開いている複数 PR は、統合コスト（未収束 PR への引きずられ + 統合 head の再レビュー 1 巡）が節約を上回るなら統合しない — 束ねるのは次の編成タイミング以降に見つかった issue からでよい。
...
この束ね方針は DORA の small-batch 推奨と逆方向だが、Actions 課金の実測（PR 固定費支配）に基づく意図的トレードオフである（[#2239](https://github.com/Dayopt/dayopt/issues/2239)）。
...
いずれも §なぜ束ねるか と同じ理由（Actions 課金は PR ごと・push ごとの固定費が支配的）から来る規律で、束ねの単位を「PR」から「PR 内の push」まで一段細かくしたもの。
...
- **draft 中に走るもの**: Actions では何も走らない（2026-08-28、#2483。CI ファイル統合で旧 docs-guard.yml（gitleaks を含む secret scan + docs 整合性チェック）は `ci.yml` の static job へ吸収され、**Docs Guard 単独では draft 中も走らなくなった** — static job 自体が draft skip の対象のため）。**Actions の外では Vercel の Preview build（`Vercel – product` / `Vercel – web`）が draft でも push ごとに走る**ため、product / web の `next build` と bundle 検査（§build と bundle 検査は Vercel 側で走る）は draft 中も効いている。draft 中の Actions 側の機械ゲートは pre-push フック（§Pause point）のみ
- **ready 後に走る層**: `ci.yml` の static job（gitleaks + secrets:check + docs:check + typecheck/lint/knip の並列 lane）→ test job（unit + affected integration/RLS + migration safety）の直列 2 job（#2483 で 1 ファイルへ統合。static が落ちたら test は走らない fail-fast）+ Production Config Audit（保護対象該当時のみ trusted dispatch）。ready 化で `ci.yml` 自体が新規発火する（`types` に `ready_for_review` を明示済みのため、旧 Docs Guard が抱えていた「ready 化で再発火しない」特例は解消済み）。**E2E / Web E2E は per-PR から撤去済み**（2026-08-20、CI 4 層再設計 #2269。§CI 4 層構造（2026-08-20 改訂） 参照）
- **flow（2026-08-28 改訂、#2483）**: 「draft で push（CI は完全 skip。手応え確認は pre-push フックとローカル `pnpm check`）→ レーンが自己判断で ready 化 → **ready 化で static→test 直列 CI が起動** → green 確認 → 指揮台へ「レビュー待ち」報告 → クロスレビュー → 指摘があれば **ready のまま** 1 round = 1 push で fix → thread 全 resolve + green で `pnpm branch:finish`」。`branch:finish` は draft を拒否する（既存挙動）
- **skip は検査の免除ではない。** draft 中の static / test job は `conclusion: skipped` の check run を発行し、`branch:finish` が「docs-only でない PR では両者が success であること」を名前で要求する（`scripts/tasks/finish-branch.sh`、#2415）。**ただしこの要求は `branch:finish` 経由でのみ効く。** GitHub の merge ボタンと手動 `gh pr merge` は素通りする（GitHub は skipped の required check を success 扱いにし、private repo + Free plan では ruleset の強制自体が効かない）——§内製クロスレビューの実施を要求する gate と同じ制約で、**この改訂は「ready 化直後の窓を素通りで merge した場合」の中身を検証済みから未検証へ変えている**。merge の入口を `branch:finish` に一本化する運用（§マージ手順）がこの前提
...
2026-08-03 実測: 3 日間で CI 38 run / 15 PR。push 2.5 回に対して merge 前に必要な全量検証は 1 回で、全 push で重量層まで走らせると月 ~11,000 課金分ペースだった。同じ原理で Integration Tests の push:main トリガー（当時は up-to-date gate により PR 検証と同一 tree の再検証だった）を廃止し、Production Config Audit（Vercel 側 drift の検査で PR diff と無関係）を draft skip + 日次 cron に変えた。**2026-08-20 の改訂（[#2263](https://github.com/Dayopt/dayopt/issues/2263)）で「ready 化は merge 直前に 1 回」の運用は終了し、fix round のたびに重量層が再走することを明示的に許容している**（上記フロー参照）。**2026-08-26 の改訂（[#2415](https://github.com/Dayopt/dayopt/issues/2415)、User 裁可）で draft 中の軽量層も廃止した。** 論拠は「検査網が要るのは境界を越える瞬間（ready = 指揮台の時間を使う / merge = main に入る）だけで、draft はレーンの私的作業場。軽量検査は pre-push フック（#2359）がローカルで毎回実施済みのため二重払い」。これは指揮台の推奨（draft 中も維持）と分かれた判断で、`judgment:diverged` として記録し月次 gardening が実測で正否を判定する（判定観点 = 復活条件。`.github/workflows/ci.yml` 冒頭コメント）。**2 段階構造そのものは残る**が、境界は「軽量 / 重量」ではなく「draft = ローカルのフックのみ / ready = CI」へ移った。

### CI 4 層構造（2026-08-20 改訂、2026-08-28 に #2483 でファイル構成を再改訂）

private 化前提の確定（2026-08-20 の決定ログ（削除済み、git 履歴参照））により、per-PR で E2E / Web E2E を走らせるコストが Actions 予算を圧迫すると判明したため、CI を 4 層へ再設計した（[#2269](https://github.com/Dayopt/dayopt/issues/2269)）。**層構造の考え方（draft は境界の外・ready と merge が境界）は 2026-08-26（[#2415](https://github.com/Dayopt/dayopt/issues/2415)）から不変**だが、**ファイル構成は 2026-08-28（[#2483](https://github.com/Dayopt/dayopt/issues/2483)、CI ファイル統合 Phase 1）で 11→5 ファイルへ再編した**。旧 docs-guard.yml / heavy-post-merge.yml / integration.yml / night-watch.yml / replica-check.yml / storage-backup-export.yml / issue-status-label-cleanup.yml は撤去され、内容はすべて `ci.yml` または新設 `nightly.yml` へ吸収されている（検査内容自体は削っていない）。

| 層  | タイミング | 内容                                                                                                                                                                                 | 実装                            |
| --- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------- |
| 1   | draft push | Actions では何も走らない（完全 skip）。機械 gate は pre-commit / pre-push フックのみ。Actions 外では Vercel Preview build が draft でも走る                                          | 該当ファイル無し                |
| 2   | ready      | `ci.yml`: static job（gitleaks + secrets:check + docs:check + typecheck/lint/knip、impact 判定も兼ねる）→ test job（unit + affected integration/RLS + migration safety）の直列 2 job | `.github/workflows/ci.yml`      |
| 3   | 下記参照   | heavy-e2e（🎭 E2E Tests）/ heavy-web（🌐 Web Build & E2E）/ integration（Integration Tests）の 3 job                                                                                 | `.github/workflows/nightly.yml` |
| 4   | promote 前 | promote.yml 内の smoke（既存）+ 層 3 green を promote の前提条件に                                                                                                                   | `.github/workflows/promote.yml` |

**層 3 の 3 job は 1 つの `nightly.yml` に同居するが、トリガーの粒度は job ごとに非対称**（2026-08-25、[#2382](https://github.com/Dayopt/dayopt/issues/2382)。per-merge 実行のコストを実測し、job ごとに残す価値を判定し直した結果であり、揃え忘れではない。#2483 でファイルは統合されたが、この非対称設計自体は変えていない）:

| job（旧ファイル名）                              | nightly | 手動発火（`-f jobs=layer3`） | main push 後                                                                                                                                                    |
| ------------------------------------------------ | ------- | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| heavy-e2e / heavy-web（旧 heavy-post-merge.yml） | ✅      | ✅                           | ❌（2026-08-25 廃止。E2E 約 5 分 + Web 約 1 分 × 1 日約 7 merge ≈ 月 1,300 分が無料枠 2,000 分/月の 6 割超を占めていた）                                        |
| integration（旧 integration.yml）                | ✅      | ✅                           | ❌（#2483 で per-PR 化——DB を触る PR のみ `ci.yml` の test job が affected 判定で実行する形へ移行し、旧 `paths` フィルタ付き push:main トリガー自体を廃止した） |

**push:main トリガーが integration からも消えたのは #2483 での設計変更**（旧テーブルは「integration.yml だけ push:main を残す」非対称だったが、この push:main は per-PR 化により不要になった——DB / migration / RLS 系ファイルを触った PR は `ci.yml` の test job が直接検証するため、main push 後の遅延検知に頼る必要が無くなった）。nightly の integration job は今も残るが、**役割は「per-PR 検証をすり抜けた回帰の nightly fallback」に純化した**。
...
- **赤い main が最大 1 日残ることを受け入れる変更である。** 本番は main から自動デプロイされない（`promote.yml` は `workflow_dispatch` のみ）ためユーザーには届かない。1 日約 7 merge ペースなので、夜の赤の容疑者は約 7 コミット
...
策定日: 2026-08-17（[#2097](https://github.com/Dayopt/dayopt/issues/2097)）

`CLAUDE.md` §シンプルルール（判断層）と同じ「迷った瞬間に戻る」判断層のレビュー版。**指摘の採否・severity 判定・往復を打ち切るかどうかを判断する時にだけ**使う。この 3 箇条は書き上がった規約ではなく、使いながらブラッシュアップする。重さと速さの読み方は `CLAUDE.md` ルール 4「不可逆だけ遅く、可逆は速く」を継承する（複製しない）。

| #   | ルール                                                         | 種別 |
| --- | -------------------------------------------------------------- | ---- |
| 1   | 壊れる筋書きを語れないなら、指摘しない。語れたなら、黙殺しない | 境界 |
| 2   | merge の基準は完璧ではなく、main より安全                      | 停止 |
| 3   | 迷ったら、点を塞ぐより class を閉じる                          | 方法 |

- **ルール 1 と P1 インフレ抑止**: ルール 1 の入口は到達可能な failure scenario を構成できることで、severity は問わない（P2 もこの入口を通る）。**P1 と数えてよいのは**そのうち本番でユーザー影響・データ破壊・認可漏れ・誤課金に至る筋書きに限る。この diff 以前から壊れている既存問題や、本番挙動を壊さない記述不足（runbook の説明不足など）は P1 に計上しない。P1/P2/P3 の定義そのものの生きた正本は `.claude/skills/pr-cross-review/SKILL.md` 手順 4 で、本節では複製しない
- **P3 の運用**: P3 は次ラウンドへの同乗はよいが、**P3 のためだけに次ラウンドを起こさない**
- **ルール 2・3** はそれぞれ §同型指摘の打ち切り の「main より安全」「点の追加ではなく class ごと閉じる設計への転換」の要約。詳細判断はあちらを参照する（複製しない）
- 参照実績（実際にこの 3 箇条を根拠に判断したか）は月次ガーデニングの検証対象に加える。ルールと違う判断をした時は理由を一文残す
...
**打ち切ってよいのは「点の追加」から「class ごと閉じる設計」へ転換した時**（2026-08-11 追記）。2 ラウンド連続という回数は必要条件であって十分条件ではない。**次の 1 件を塞ぐ修正を積み続けている限り、まだ打ち切り条件に達していない。** 同日に 3 レーンが独立に同じ型でこの転換に到達した:

| 点の追加（打ち切れない）       | class ごと閉じる設計（打ち切れる）                             |
| ------------------------------ | -------------------------------------------------------------- |
| 秘密情報の検査を 1 段ずつ塞ぐ  | 参照経路の全段を塞いで fail-open の余地を無くす                |
| 特定の型のキーだけ pin する    | prefix 配下を網羅的に走査し、未分類を型によらず failure にする |
| 迂回されたコマンド名を列挙する | denylist をやめて allowlist にする                             |

転換できるなら、境界の明文化より先にそれをやる。**構造的に閉じられない**と確認できた時にだけ（TOCTOU 型のように、閉じたつもりの 1 段深くに同型指摘が構成できる場合）、境界の明文化で打ち切る。

**打ち切り後でも、より良い構造的解が出たら再開する。** 打ち切り宣言はその時点の最善の判断の表明であって、自分を縛る誓約ではない。迂回形の数え上げではなく**設計の転換**を促す指摘なら、宣言後でも採用する。手続きを守って中身を損なわない。

**本体の価値が確定したら、防御の上乗せは issue へ送る。** 打ち切りのもう 1 つの型。PR の主目的（例: 危険な迂回口の廃止）が確定した後も、その周辺の防御を強める指摘（guard の正規表現をより厳密に、等）は際限なく出せる。判断基準は「**merge した時点で main より安全か**」。安全なら、上乗せ部分に残る穴は speed bump の穴であって、閉じた境界の後退ではない。残りは別 issue で追い、PR は閉じる。本体が 3 ラウンドで確定していたのに上乗せで 6 ラウンドまで伸びた事例がある（2026-08-11）。この収束はレーン任せにせず指揮台が主導する（`.claude/rules/orchestration.md` §1 日サイクル）。

**明文化した境界が誤っていたと気づいたら撤回する。** 境界は「以後それを根拠に反論する」ための道具なので、**誤った境界は境界が無いより危険**で、誤りを正当化する根拠として機能してしまう。「A をすべて閉じた」と書いたが実際には A の一部しか閉じていなかった、という取り違えが実際に起きている。撤回は打ち切りそのものの取り消しで、そこから 1（fix を積む）へ戻る。

#### plan-review の収束判定

策定日: 2026-08-11

**plan-review の多巡を止める条件は「新規の実欠陥がゼロになったか」ではなく「アーキテクチャが巡を跨いで安定したか」。** 前者は到達不能なゴールで、局所的な実装細部（分類の閾値、報告経路、copy の 1 キー、guard の実装形式）は何巡回しても数件出続ける。骨格が 2 巡連続で不変なら、残りが細部だけであることを確認して凍結する。
