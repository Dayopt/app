...
夜勤 Routine（`night-watch` v2、07:00 JST 実行。[#2291](https://github.com/Dayopt/dayopt/issues/2291) で朝の別 Routine を新設せず夜勤へ統合）が毎日「盤面 YYYY-MM-DD」issue（`type:board` ラベル）を起票する。**起票テンプレの正本は `.claude/skills/dispatch/SKILL.md` 操作C（日次棚卸し）、実行手順の正本は `.claude/skills/night-watch/SKILL.md` §自動パート Step 1**（いずれも複製しない）。本節はこの issue を指揮台がどう使うかだけを扱う。
