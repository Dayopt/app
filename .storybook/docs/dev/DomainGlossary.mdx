...
<Meta title="Architecture/Domain Glossary" />

# Dayopt ドメイン用語集

Dayopt固有のドメイン概念とコードベースで使用される用語の定義。

> Storybook用語（CSF, Meta, Story 等）は [Storybook 公式用語集](?path=/docs/docs-storybook-glossary--docs) を参照。
...
- ADR: [ADR-005 時間不変原則](?path=/docs/docs-architecture-adr-005-時間不変原則--docs)
...
- ADR: [ADR-001](?path=/docs/docs-architecture-adr-001-統合ブロックモデル--docs), [ADR-005](?path=/docs/docs-architecture-adr-005-時間不変原則--docs)
...
- ADR: [ADR-002](?path=/docs/docs-architecture-adr-002-feature-slicedアーキテクチャ--docs)
