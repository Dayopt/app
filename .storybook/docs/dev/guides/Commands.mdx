...
> **注意**: `npm run dev` は `vercel env pull` を自動実行して `.env.local` を最新化する。
...
```bash
npm run dev                 # vercel env pull + next dev
...
npm run types:generate      # 本番から型生成
npm run types:generate:staging  # Stagingから型生成
npm run types:generate:local    # ローカルから型生成
```
