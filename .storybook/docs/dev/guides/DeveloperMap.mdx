...
├── styles/tokens/             # デザイントークン + Storybook可視化
│
├── stories/
...
| デザイントークン変更     | `src/styles/tokens/`      | `colors.css`, `spacing.css`           |
...
| Supabase 型確認      | `src/lib/database.types.ts`                    | `npm run types:generate:staging` で再生成 |
