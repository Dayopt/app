...
<Meta title="Architecture/Service/Audit" />

# Service 層 Audit（現状の歪み地図）

調査日: 2026-05-01（main: `52ef53d77` 時点で verify 済み）
target shape: [Service Contracts](?path=/docs/docs-architecture-service-contracts--docs)
...
detail は [Service Contracts](?path=/docs/docs-architecture-service-contracts--docs) の Delta section を参照。
