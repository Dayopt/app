...
> [ADR-021](./2026-06-22-shared-packages-canonical-and-app-shims.md) を前提に、`packages/components/src/` の第二階層 category を精緻化する。[ADR-021](./2026-06-22-shared-packages-canonical-and-app-shims.md)
...
[ADR-021](./2026-06-22-shared-packages-canonical-and-app-shims.md) で `packages/components` を canonical 実装＋単一 barrel に集約した際、第二階層は
...
基底のみ `@dayopt/components` から取得する。[ADR-021](./2026-06-22-shared-packages-canonical-and-app-shims.md) の「i18n 結合の強い component は
...
- [ADR-021](./2026-06-22-shared-packages-canonical-and-app-shims.md) — デザインシステム共有レイヤー（packages canonical / app 直接 import）。本 ADR は
  その第二階層 category を精緻化する
- `packages/components/src/index.ts` — 単一 barrel（category 別に整理）
- `.claude/rules/feature-boundaries.md` — app 内 feature 境界（packages ↔ app 境界は [ADR-021](./2026-06-22-shared-packages-canonical-and-app-shims.md)）
