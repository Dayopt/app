...
- 2026-09-03: [db] epic #2162 の Step 9 として `public.tags` / `plans.tag_id` / `records.tag_id` / tags 専有 10 関数を物理削除し、同一 PR で読み取り経路の残存参照除去と tag 語彙の activity への改名（#2389）まで行う（理由: 観測窓 10 日で Sentry ヒット 0、production の MCP receipt / OAuth connection とも 0 件で digest pin の前提も消滅し、EXPLICIT AUTHORITY の裁可が揃ったため。失うのは tags 6 行と timeblock 67 件の凍結 tag_id で、PITR 無効のため復旧粒度は日次スナップショットのみ）（参照: [#2175](https://github.com/Dayopt/dayopt/issues/2175)）
