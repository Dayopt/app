...
カレンダー上のオーバーレイ（予定と実績が同じタイムラインに重なって見える画面）。1 Entry モデルの視覚的ペイオフであり、スクリーンショット1枚で差別化が伝わる（[experience-design.md](./experience-design.md)）。統計/インサイト画面は補助的に添える程度に留め、メインでは使わない（レポート機能の印象を強めすぎない）。
...
- **精度スコア**: 意図と実行のギャップを数値化。「鏡」としての位置づけを維持（[experience-design.md](./experience-design.md) の作成時フィードフォワードの実装）
