...
**月次解約率** = 当月中に解約した Pro ユーザー数 ÷ 月初の Pro ユーザー数。データソースは Stripe。[cost-optimization.md](cost-optimization.md) の BEP 試算は 5%/月 を前提としており、この定義に固定する。コホート単位のリテンション曲線は当面追わない（ユーザー数が少ないうちは月次率で十分）。

**LTV / CAC は意図的に追わない。** 有料広告を使わない前提のため CAC は実質 0 であり、比率管理は意味を持たない。事業の持続可能性は粗利率とチャーン率で見る。

## プロダクト内で見せる指標との関係

ユーザーに見せる進捗指標は事業 KPI とは別系統。一次証拠は**見積もり精度の推移**（過去の自分との事実比較。[concept.md §4-6](../product/concept.md)）であり、ストリーク・点数・ランク等の gamification 指標はプロダクトにも事業 KPI にも存在しない。
