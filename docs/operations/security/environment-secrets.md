...
- **監視対象は live 応答の全数トリアージから起こす。** 現在 21 件を pin し、`security_` / `hook_` / `mfa_` / `sessions_` / `password_` の boolean で契約にも除外リストにも無いものが現れたら failure にする。`external_*`（95 キー）と `mailer_*`（40 キー）は provider / template が増えるたびにキーが増え、キーの存在自体は危険ではないため guard の対象外にし、危険な値だけ個別に pin する
