...
| `SUPABASE_STORAGE_RLS_AUDIT_TOKEN` | Production Storage RLS Audit                 | 1Password `ci/supabase-storage-rls-audit`（field `credential`）の replica。`database_read` のみの scoped token。storage-rls job の 1 step だけへ渡す。**2026-08-24 時点で未発行**（発行手順は [#2345](https://github.com/Dayopt/dayopt/issues/2345)）。未設定の間、job は `::notice::` を出して exit 0 になる |
