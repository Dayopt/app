...
- **GitHub Actions の job log** — run の結論を `night-watch: <verdict> | 観測 N/7 | 起票 N | 保留 N | 見送り N | 取得失敗 N` の 1 行で残す
...
このチェックは毎朝の義務ではない。夜勤の異常は alert issue として能動的に届くので、run 自体の死活が疑わしい時（issue が長く 1 件も来ていない・Actions の障害情報がある等）に見ればよい。
