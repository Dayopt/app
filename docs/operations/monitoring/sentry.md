...
**アラート設定の詳細手順**: [Docs/Guides/SentryAlerts](sentry-alerts.md)
...
Sentry の値は 1Password master に保存し、`.op-env.local` の `op://` 参照で注入する。schema と同期手順は `docs/operations/secrets.md` を参照。
...
推奨アラートルールの詳細は [SentryAlerts](sentry-alerts.md) を参照。
...
- **アラート設定**: [SentryAlerts](sentry-alerts.md)
