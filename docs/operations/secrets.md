...
local dev と、MCP の一時的な PR Preview に使う。常設 Staging は作らない。Preview では、検証対象 PR 専用の Supabase と Upstash だけを一時的に複製し、Production の値は使わない。
...
`google-calendar` は外部カレンダー取り込み（[#1702](https://github.com/Dayopt/dayopt/issues/1702)）専用の OAuth client で、Supabase Auth の Google provider とは別 client として作る。Supabase 側の client secret を流用しない。`GOOGLE_CALENDAR_PROJECT_NUMBER` は client ID の先頭にある project number と一致させ、local / Staging / Production で別の Google Cloud project を使う。
...
- `MCP_OAUTH_ENVIRONMENT`はOAuth identityの環境marker。一時Previewでは`preview`を必須とし、`VERCEL_ENV=preview`、`VERCEL_TARGET_ENV=preview`、branch、issuer、resourceのどれかが一致しなければbuildとruntimeを停止する。Productionは未設定時だけ既存originを既定値にする
...
- `STRIPE_ACCOUNT_ID`と`STRIPE_LIVEMODE`は、アカウント削除が正しいStripe account・modeだけを変更するための固定identity。`STRIPE_SECRET_KEY`と3項目をまとめて設定する。test modeは`STRIPE_LIVEMODE=false`、live modeは`true`
- MCPの一時PreviewにはVercelの標準Previewを使い、Custom Environmentや常設Stagingは作らない。ProductのResend、Sentry、Stripe、billing webhook環境変数はPreviewへ複製しない
