...
| ラベル     | ja     | アクティビティ | CI 必須        | キー `[Aa]riaLabel$` / キー `^aria\\.`                                                            | 1 対象に複数付けられる印象を与える。1 タイムブロック 1 アクティビティ。a11y の aria-label は別義                              |
...
- **「ラベル」は値だけの禁止** — a11y の `aria-label` は別義なので、キー名の `ariaLabel` / `aria.*` は除外する
