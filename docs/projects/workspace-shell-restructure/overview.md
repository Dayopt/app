...
### 5-7. モバイルのタブ切替 — v1 の推奨（決定は指揮台経由で User）
...
| 案                                                            | 評価                                                                                                                                                                           |
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| (a) `BottomTabBar` を 2 タブで復活                            | 不採用。2 度削除された資産（`cf79907eb` / `66a3ea6db`）の復活で、しかも `ActivityChipRow` の固定フッターと画面下端を奪い合う。クイック作成は「一手少なく」の中核なので譲れない |
| (b) `AppHeader` にセグメントコントロールを足す                | 次善。ただしカレンダーは `hasOwnHeader` で `AppHeader` を出さない（`mobile-layout.tsx`）ので、カレンダー独自ヘッダー側にも同じものを作る必要があり二重実装になる               |
| **(c) #2161 が作った既存トグルを `/report` 遷移へ付け替える** | **推奨**                                                                                                                                                                       |
| (d) v1 はデスクトップのみ、モバイルは `/report` へ行けない    | 不採用。分析を見る動機が最も強いのは移動中で、そこで到達できないのは機能の欠落                                                                                                 |

**(c) を推す理由**: #2161 がカレンダーヘッダーに `CalendarPanelToggle`（統計 / 差分のセグメント）を作ったばかりだが、**パネルが廃止される以上このトグルは行き先を失う**。捨てずに `/report` への遷移へ付け替えれば、新しい chrome を 1 つも足さずにモバイルの入口が立つ。#2161 が「段階統合 Phase 1」と名乗っていた統合の Phase 2 が、まさにこれにあたる。

戻り経路: `/report` はカレンダーと違って独自ヘッダーを持たないので、モバイルでは標準 `AppHeader` が出る。そこへ戻る導線を置く。

デスクトップでは Sidebar タブが主導線なので、**デスクトップのヘッダーからはこのトグルを外す**（同じ機能への入口を 2 つ持たない）。
