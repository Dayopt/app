...
| 契約                    | 現状                                                                                    | 扱い                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ----------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OAuth scope             | `read:tags`（`SUPPORTED_SCOPES` / `ADVERTISED_SCOPES` の両方に載る）                    | **`read:activities` へ alias なしクリーン置換**（実装確定、2026-08-18）。当初は「`read:activities` を追加し `read:tags` は同義の deprecated alias として残す」方針だったが、H2 の production 実測で `oauth_connections` / `oauth_tokens` / `oauth_authorization_codes` の 3 テーブルが全件 0 行（= 接続クライアントが 1 つも存在しない）と判明し、alias 維持が守る対象そのものが無かったため User 裁可 c（実測 0 件ならクリーン置換）どおりに実施した（§12 で再掲、migration `20260818150000`） |
...
**この節が想定していたリスクは実測で不成立になった（2026-08-18、H2 着手時）。** production を read-only で実測したところ、`oauth_connections` / `oauth_tokens` / `oauth_authorization_codes` の 3 テーブルが**全件 0 行**だった。上記の壊れ方（all-or-nothing 401）を踏む接続そのものが 1 つも存在しないため、alias 維持は「母数が空のリスクを避けるコスト」になった。User 裁可 c（実測 0 件ならクリーン置換）どおり alias なしで置換した（`20260818150000`）。この節の分析自体は誤っていない — 接続が 1 件でもあれば alias 維持が正しい判断だった。
...
| 8   | **tag_id 剥離** — additive-only（Step 3）で残した 15 関数の `p_tag_id` 引数・`tag_id` 列参照・15 TS ファイルの `tagId` 読み書き・検索クエリの `tags` 参照を除去し、DB書き込み経路から tags を完全に切り離す（テーブル・列自体は残す）                              | issue #2352                     | `[hours]`                         | 2026-08-24 に発覚した前提欠落（#2175）の補完 Step。詳細は §Step 8（tag_id 剥離）の設計                                                                                                                                                                                                 |
| 9   | **destructive migration** — `tags` テーブル・`tag_id` 列・tag 専有 RPC / トリガー群 drop（孤立 10 関数の先行 drop は行わず本 Step へ同乗）+ **`delete_all_user_data_command` 系の `DELETE FROM public.tags` 行を同一 migration で除去**（§Step 9 の前提条件 参照） | 未割当                          | `[days]`                          | **`EXPLICIT AUTHORITY`**。Step 8 の完了 + 剥離後の観測窓の再計測 + 明示指示 + 独立レビュー + backup / PITR 確認が揃うまで実行しない                                                                                                                                                    |
...
- **15 の関数**が `p_tag_id` 引数または `tag_id` 列参照を持つ。**直接**（自身の SQL 本体に `p_tag_id` 引数または `tag_id` 列参照を持つ）14 本 + **間接**（自身に参照は無いが直接参照を持つ関数を呼ぶだけの wrapper）1 本に分かれる（クロスレビュー指摘で訂正、当初「13 本」としていたのは誤り。#2352 のチェックリストもこの内訳に同期させる）:
  - 直接 14 本: 4 public wrapper（`create_plan_command_v1` / `update_plan_command_v1` / `create_record_command_v1` / `update_record_command_v1`、`p_tag_id` 引数）+ 4 private 実体（`private.{create,update}_{plan,record}_unserialized_v1`、`p_tag_id` 引数）+ 4 `apply_mcp_*_v1`（`p_tag_id` / `p_tag_id_present` 引数）+ `private.record_plan_unserialized_v1`（引数には無いが本体で `v_plan.tag_id` を `records.tag_id` へ直接コピー、Plan→Record 変換。#7-2 参照）+ `private.confirm_day_plans_unserialized_v1`（同型のコピーを行う一括確定バッチ）
  - 間接 1 本: `record_plan_command_v1`（public wrapper。自身の本体に `tag_id` 参照は無く、直接参照を持つ `private.record_plan_unserialized_v1` を呼ぶだけ。剥離時は呼び出し先のシグネチャ変更に追従すれば足り、自身の引数変更は不要）
- **15 の TS ファイル**（`plan-service.ts` / `record-service.ts` / `timeblock-command-client.ts` / `timeblock-command-service.ts` / `mcp-mutation-client.ts` / `mcp-mutation-db.ts` / `TimeblockInspectorForm.tsx` / `timeblock-day-diff.ts` / `timeblock-duplicate.ts` / `timeblock-clipboard.ts` / `timeblock-menu-items.ts` / `build-day-diff-inputs.ts` / `types/plan-event.ts` / `types/record-event.ts` / `schemas/timeblock.ts`）が、create 時は `tagId: null`、update 時は `tagId: existing.tag_id` を毎回 RPC へ再送している（`plan-service.ts:175,229` / `record-service.ts:167,226` で実測）。UI に tag を新規割り当てる導線は無い（`TagRow.tsx` / `TagQuickSelector.tsx` は #2258 で撤去済み）が、**既存の tag_id を持つ過去ブロックを編集するたび同じ値をそのまま echo している**（`schemas/timeblock.ts:36,75` の `tagId` は当初の凍結リストから漏れていた。Codex レビュー指摘、#2349 試運転 1 件目で訂正）
...
- DB: 直接参照を持つ 14 本を「旧シグネチャを厳密指定して `DROP` → `p_tag_id` / `tag_id` 参照を除いた新シグネチャで `CREATE`」する（Step 3 と同じ additive-only の逆操作。GRANT / REVOKE の再適用も同様に必須、§Step 3 の RPC 変更は additive-only に固定する 参照）。間接の `record_plan_command_v1` は呼び出し先の変更に追従するだけで自身の drop は不要。`enforce_plan_tag_owner` / `enforce_record_tag_owner` トリガーは、列がまだ残っていても `tag_id` を書く経路が無くなれば `UPDATE OF tag_id` 条件が発火しなくなるため、この Step で trigger ごと drop してよい（tags テーブル自体の存続とは独立）
- TS: 15 ファイルから `tagId` フィールドを削除し、型・呼び出し引数を simplify する。実装時の実測では `p_tag_id` を直接呼び出す integration test は当初想定の 5 本を超えて 9 本（`activity-assignment-guards` / `calendar-account-deletion-app` / `mcp-plan-create-apply` / `mcp-plan-mutations-apply` / `mcp-record-mutations-apply` / `mcp-stage1-rollout-compat` / `mcp-stage1-writer-fence` / `record-fulfillment` / `rls-access`（false positive、`p_tag_ids` 系の別 RPC のみ・要修正なし）/ `timeblock-atomic-commands`）に及んだ。`tag-assignment-guards` / `mcp-tag-tristate-guards` の 2 本は検証対象（`assert_active_timeblock_tag_v1` 呼び出し・`enforce_*_tag_owner` トリガー）が本 Step で消滅したため削除し、`activity-assignment-guards` が同等カバレッジを引き継ぐ（issue #2352 実装コメント参照）
...
**Reversibility**: `[hours]`。RPC の DROP→CREATE と trigger drop は DB migration を含むため（`plan-format.md` の定義で `[minutes]`＝純粋な code 変更、`[hours]`＝migration を含むが 1 日以内に rollback 可能）、production 適用後の巻き戻しは `git revert` ではなく逆向き migration（旧シグネチャの再作成 + ACL 再適用 + trigger 再作成）が必要（クロスレビュー指摘で訂正、Step 3 が同型作業を `[hours]` としているのと整合させる）。TS・検索クエリ部分のみなら `[minutes]` だが、RPC/trigger の migration 部分が支配する。テーブル・列は温存されるためデータ損失は伴わない。

**deploy skew の注記（risk-reviewer 指摘、merge 判断に必要）**: migration は main merge 時に自動適用される一方、Vercel deploy（新コード反映）は並行して数分遅れて着地する（`workflow.md` §CI 4 層構造）。この窓では「旧コード（`p_tag_id` を送る）× 新 RPC（`p_tag_id` を受け付けない）」の組み合わせが生じ、Plan/Record の作成・編集・MCP write が `PGRST202`（該当シグネチャなし）で全滅する。逆順（deploy 先行）でも新コードが旧関数に一致せず同型の失敗になるため、**ゼロダウンタイムになる migration 順序は存在しない**（`architecture.md` の「コード側削除を先に production へ deploy → 静穏確認 → drop migration」という通常手順を、この Step は踏めない）。

`decision-principles.md` ルール 1「破滅に賭けるな」に照らすと: 影響は自己修復的（Vercel deploy 完了で自動回復、通常 2〜5 分）・データ損失なし・単一ユーザー・課金前（`paid-billing-not-enabled-yet` 実測）。`§12-3` が同種の deploy skew（OAuth scope 側）を「可逆で影響範囲が限定的」として運用注記で打ち切った前例と同じ扱いが妥当と判断する。**この受容は merge 実施者（指揮台）が merge 直前に再確認する CHECKPOINT とし、低トラフィック時間帯に merge し、Vercel deploy が green になるまで新規の Plan/Record 操作・MCP write を試さない運用注記を merge 手順に添える。**

**MCP 冪等 digest の 90 日互換窓（Step 9 の前提条件へ追加）**: `apply_mcp_plan_create_v1` / `apply_mcp_record_create_v1` の冪等 digest は `'tagId'` キーを固定 `NULL` として残す（20260824095705、旧クライアントは常に `p_tag_id: null` を送っていたため digest がバイト一致する）。`private.resolve_mcp_mutation_replay_v1` の receipt 保持期間は 90 日のため、**Step 8 適用から 90 日が経過し、Step 8 以前に発行された `mcp_mutation_receipts` が全て期限切れになるまでは、この固定 `'tagId': NULL` キーを digest から外してはいけない**。Step 9 着手前提条件に追記済み（下記）。

### 逆向き migration（rollback）手順

策定日: 2026-08-24（risk-reviewer 指摘、#2352。実 DB への逆適用リハーサルは求めない — Reversibility `[hours]` の判断根拠のみ明文化する）

Step 8 適用後に巻き戻す場合、以下を 1 本の新規 migration（既存 migration ファイルは書き換えない）で行う:

1. **DROP** 12 関数の Step 8 後シグネチャ（`create_plan_command_v1` 等 8 本 + `apply_mcp_plan_create_v1`/`apply_mcp_record_create_v1` の digest 修正後シグネチャは同一のため対象は変わらず 12 本）
2. **CREATE** 各関数を Step 8 前の完全シグネチャ（`p_tag_id` / `p_tag_id_present` を含む）で再作成。本体は `20260824090000` 冒頭コメントが引用する `pg_get_functiondef` dump（このファイル自体は commit 履歴に残る）を正本として復元する
3. GRANT/REVOKE を Step 8 前の ACL（`private.*` は owner-only、`public.*` は `service_role` へ GRANT）で再適用
4. `enforce_plan_tag_owner()` / `enforce_record_tag_owner()` 関数を再作成し、`plans` / `records` へ CONSTRAINT TRIGGER として再アタッチ（`CREATE CONSTRAINT TRIGGER ... AFTER INSERT OR UPDATE OF user_id, tag_id ... DEFERRABLE INITIALLY IMMEDIATE FOR EACH ROW EXECUTE FUNCTION ...`）
5. TS 側は該当 commit を `git revert` する（migration の逆適用と TS revert は同一 PR で揃える）

**巻き戻しの制約**: Step 8 適用後に新規作成された Plan/Record の `tag_id` は Step 8 期間中ずっと `NULL`（書き込み経路が受け付けないため）。ロールバックしても過去に遡って `tag_id` を補完することはできない（データ損失ではないが、機能停止期間中のデータは tag なしのまま）。
...
- **Step 8 の production 適用から 90 日以上経過していること**（`private.resolve_mcp_mutation_replay_v1` の receipt 保持期間。Step 8 は `apply_mcp_plan_create_v1` / `apply_mcp_record_create_v1` の冪等 digest に `'tagId': NULL` を固定キーとして残しており、Step 8 以前発行の receipt が全て期限切れになるまではこのキーを外せない。Step 9 で `tags` テーブルを drop するタイミングでこの固定キー自体を削除する新規 migration も同時に用意する）
- backup / PITR の存在確認
- drop 対象の完全シグネチャと依存を catalog（`pg_depend`）で確認してから drop する（time-model-split Step 9b と同じ手順）
- **`delete_all_user_data_command` 系（実測時点の最新版。2026-08-24 時点は `v5 → v4` の呼び出し経路）から `DELETE FROM public.tags` 行を、`tags` テーブルを drop する同一 migration 内で除去する**（クロスレビュー指摘、P1）。GDPR 削除関数は「tag 専有」ではないため上記の drop 対象カタログには入らず、Step 8（テーブル温存が前提）でも触ってはいけない。素通りすると、`tags` drop 後の次回アカウント削除が `relation "public.tags" does not exist` で失敗する。plpgsql 本体内の参照は `pg_depend` の catalog gate では検出できないため、実装時に手動でこの関数を確認対象に含める
...
- ~~`read:tags` scope を即座に削除しない（§7-4）。alias で受ける~~ → **実装確定で撤回**（2026-08-18）。production 実測が 0 件だったため、§7-4 の更新どおり alias なしクリーン置換を実施した
...
| c   | `read:tags` scope            | **実装確定（2026-08-18）**: H2 着手時の実測で production の grant が 0 件（`oauth_connections`/`oauth_tokens`/`oauth_authorization_codes` 全件 0 行）と判明し、alias なしクリーン置換で確定 | §12-3 は「設計としては 0 件でも alias 維持を推奨」していたが、PR #2201 のクロスレビューで reviewer もこの判断に同意し、alias なしクリーン置換を採用（下記 §12-3 参照。撤回の経緯を残す）                            |
...
### 12-3. 決着済み（2026-08-18）— `read:tags` alias を 0 件でも残すか

**決定: alias を残さない（クリーン置換）。** 以下は決着前の推奨（残す）とその根拠で、記録として残す。

この節が警告していたリスクは「既存トークンが `read:tags` を持ったまま `SUPPORTED_SCOPES` から外れ、all-or-nothing gate で 401 になる」というものだった。production 実測で `oauth_connections` / `oauth_tokens` / `oauth_authorization_codes` が全件 0 行と判明し、**壊れうる既存接続が 1 件も無かった**ため、このリスクは母数ごと消えた。**接続が 1 件でもあれば、この節の推奨（alias を残す）が正しい判断だった。**

別途、H2 merge 後の deploy skew（migration と Vercel deploy が非同期に完了するまでの窓）で「新規 grant 作成」が `22023` で失敗しうることをレーン H が発見しているが、これは §12-3 が扱っていたリスクとは別物（既存接続の 401 ではなく新規接続の作成失敗）。可逆で影響範囲が限定的と判断され、コード変更ではなく運用注記（merge 実施者は deploy 完了まで新規 MCP 接続を試さない）で打ち切られた。詳細は PR #2201 のクロスレビュー thread を参照。
