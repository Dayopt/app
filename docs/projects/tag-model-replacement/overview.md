...
| 8   | **tag_id 剥離** — additive-only（Step 3）で残した 15 関数の `p_tag_id` 引数・`tag_id` 列参照・14 TS ファイルの `tagId` 読み書き・検索クエリの `tags` 参照を除去し、DB書き込み経路から tags を完全に切り離す（テーブル・列自体は残す）                              | 未割当                          | `[hours]`                         | 2026-08-24 に発覚した前提欠落（#2175）の補完 Step。詳細は §Step 8（tag_id 剥離）の設計                                                                                                                                                                                                 |
