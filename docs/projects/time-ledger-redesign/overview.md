...
- [step-2-expand-only-schema-foundation.md](./step-2-expand-only-schema-foundation.md) — [#2433](https://github.com/Dayopt/dayopt/issues/2433) の expand-only schema 基盤（複合 tenant FK・Undo substrate の構造・versioned view/RPC の規約と guard・destructive checker の UPDATE 検知）。本書§6の8段依存順の第2段。**各段へ何を残したかの棚卸しを持つ**
