...
上の制約（「その project の live SHA を基準に diff を取る」）を満たす実装として、`turbo-ignore` ではなく Vercel が Ignored Build Step 用にネイティブで露出する `VERCEL_GIT_PREVIOUS_SHA` を使う。
...
- **基準 SHA は `VERCEL_GIT_PREVIOUS_SHA`（Vercel がネイティブに提供する env）を使う。** これは「その project + branch の直前の**成功** deployment の SHA」で、Ignored Build Step 設定時のみ build container に露出する。§8 の制約が要求する「project ごとの live SHA」と一致する（自前で live deployment を問い合わせる API 呼び出しが不要）
