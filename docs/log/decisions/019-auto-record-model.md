...
ADR-011 で plans / records を単一 `entries` テーブルに統合した時点では、`origin` は `'planned'` のみで、実績は別途記録する想定だった。だが時間記録の最大の障壁は記録の手間である。Toggl 型の「開始」「停止」を能動的に押させる方式は押し忘れ・後付け修正が常態化する。
...
- ADR-011 — 統合ブロックモデル（本 ADR が拡張するデータモデル。`origin = 'unplanned'` と actual 二層を追加）
- ADR-015 — 時間不変原則（過去ブロックの actual 編集は許可、予定編集は禁止）
- ADR-018 — 時間重なり全面禁止（自動記録の防衛線分担）
