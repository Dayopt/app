...
| `src/lib/hooks/useUserPreferences.ts`                                                            | `snapInterval: 5\|10\|15\|30`（既存 union に `1` を含むかが Project A の設計論点）                             |
...
`useUserPreferences.snapInterval` は **user 設定の選択肢** であり、ここに `1` を入れると「drag を 1 分 snap にする選択肢」が UI に滲む。設計意図（drag は粗く、Inspector / text input のみ 1 分）を壊さないため、**1 分粒度は precision policy の内部定数として隔離する**。
...
`useUserPreferences.snapInterval` の union は **現状の `5|10|15|30` のまま維持**。`1` を追加しない。
...
| A-1  | `src/features/calendar/lib/precision.ts` を新設し上記 3 定数を export。`useUserPreferences.snapInterval` の union は触らない                                                                                                                                                                                                                                                                                                                                                                                                  | minutes            | none                                                            |
...
1. 本ファイルを `apps/storybook/docs/product/projects/timeline-precision-redesign/overview.mdx` に保存（保存済み）
